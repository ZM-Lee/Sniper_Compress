...
Options:
  ...
  --mqtt-topic TOPIC            Sender MQTT topic (default: ${MQTT_TOPIC})
  --mqtt-client-id ID           Sender MQTT client id (default: ${MQTT_CLIENT_ID})
  --mqtt-qos N                  Sender MQTT QoS (default: ${MQTT_QOS})
...
if [[ -n "${CAMERA_CONFIG}" && "${CAMERA_CONFIG}" != /* ]]; then
...
CAM_BIN="${ROOT}/build/camera_capture"
CPP_TX_BIN="${ROOT}/build/compress"
...
if [[ ! -x "${CAM_BIN}" || ! -x "${CPP_TX_BIN}" ]]; then
  echo "Missing sender binaries. Build first:" >&2
  echo "  cmake -S . -B build && cmake --build build -j\$(nproc)" >&2
  exit 1
fi
...
CONFLICT_RE='[p]ython -m rm_stream.gui|[c]ompress|[c]amera_capture|[p]ython .*[s]ender_msssim_qvrf'
...
Resolved stream profile:
  ...
  mqtt         : ${MQTT_HOST}:${MQTT_PORT}, topic=${MQTT_TOPIC}, tx_client=${MQTT_CLIENT_ID}, gui_client=${CLIENT_ID}
  shm          : ${SHM_NAME}
  logs         : ${LOG_DIR}
EOF
...
echo "Starting Daheng camera capture..."
CAM_CMD=(
  "${CAM_BIN}"
...
(
  ...
  while true; do
    LD_LIBRARY_PATH="${OV_LIBS}:${ROOT}/build:${LD_LIBRARY_PATH:-}" "${CAM_CMD[@]}"
    rc=$?
...
if [[ "${CODEC}" == "mbt" || "${QVRF_CPP_SENDER}" == "1" ]]; then
  ...
  if [[ "${QVRF_CPP_SENDER}" == "1" ]]; then
    RM_QVRF_CPP_SENDER=1 \
      LD_LIBRARY_PATH="${OV_LIBS}:${ROOT}/build:${LD_LIBRARY_PATH:-}" \
      "${TX_CMD[@]}" >"${TX_LOG}" 2>&1 &
  else
    LD_LIBRARY_PATH="${OV_LIBS}:${ROOT}/build:${LD_LIBRARY_PATH:-}" \
      "${TX_CMD[@]}" >"${TX_LOG}" 2>&1 &
