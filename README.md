...
CAN 接收器会监听接口上的普通 CAN 帧并打印不合法帧，但只接受一个标准 CAN 帧 ID，
默认为 `0x180`。`cmd_id` 位于 CAN 帧头的 `can_id` 字段中，不占用数据区；
接收端校验 `can_id` 通过后，再校验 DLC、方向值和填充字节，最后执行 ROI 移动。
数据区固定为 8 字节：
...
DLC 不是 8、方向值不在 1~4，或填充字节非 0 时，整条命令会被丢弃。
错误 ID、扩展帧、远程帧、错误 DLC、非法方向和非法填充都会打印实际 ID、DLC 和数据内容。

四个固定测试帧如下：

```bash
cansend can0 180#0100000000000000  # 上
cansend can0 180#0200000000000000  # 下
cansend can0 180#0300000000000000  # 左
cansend can0 180#0400000000000000  # 右
```
