...
// ---------------------------------------------------------------------------
// make_frame_message — matches Python rm_stream.message.make_frame_message()
// ---------------------------------------------------------------------------

std::vector<std::vector<uint8_t>> make_frame_message(uint32_t frame_id, const uint8_t* bitstream,
...
std::vector<std::vector<uint8_t>> make_frame_message_fixed2(
    uint32_t frame_id, const uint8_t* bitstream, int len, uint16_t flags) {
    return make_frame_message_fixed_n(frame_id, bitstream, len, FIXED_CHUNKS_PER_FRAME, flags);
}

std::vector<std::vector<uint8_t>> make_frame_message_fixed_n(
    uint32_t frame_id, const uint8_t* bitstream, int len,
    int fixed_chunks, uint16_t flags) {
    return make_frame_message_fixed_n(frame_id, bitstream, len, fixed_chunks, 0, flags);
}
...
std::vector<std::vector<uint8_t>> make_frame_message_fixed_n(
    ...
    if (fixed_chunks <= 0 || fixed_chunks > 255) {
        throw std::runtime_error("make_frame_message_fixed_n: invalid fixed chunk count");
    }
    flags = static_cast<uint16_t>(flags & FLAG_SESSION_MASK);
    int data_chunks = fixed_chunks;
    if (fec_data_chunks > 0) {
        if (fec_data_chunks >= fixed_chunks) {
            throw std::runtime_error("make_frame_message_fixed_n: invalid FEC data chunk count");
        }
    ...
    if (len > MAX_PAYLOAD * data_chunks) {
        throw std::runtime_error("make_frame_message_fixed_n: payload exceeds fixed chunk budget");
    }
...
std::vector<uint8_t> make_over_budget_message(
...
// ---------------------------------------------------------------------------
// make_strings_message — compact binary serialization matching Python _make_strings_message()
// ---------------------------------------------------------------------------

std::vector<uint8_t> make_strings_message(
...
// ---------------------------------------------------------------------------
// make_msssim_qvrf_message — matches Python msssim_qvrf_codec.make_msssim_qvrf_message()
// ---------------------------------------------------------------------------

std::vector<uint8_t> make_msssim_qvrf_message(
    const std::vector<std::string>& y_strings,
    const std::vector<std::string>& z_strings,
