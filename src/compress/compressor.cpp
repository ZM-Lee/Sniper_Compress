...
#include "compress/ga_encoder.h"
#include "compress/mbt_encoder.h"
#include "compress/preprocess.h"
#include "compress/entropy_coder.h"
...
struct compressor_s {
    // ---- MBT codec ----
    compress::GaEncoder encoder_g_a; // g_a: selectable OpenVINO or TensorRT backend
    ...
    compress::EntropyCoder entropy;
    // ---- MS-SSIM QVRF codec ----
    compress::GaEncoder encoder_msssim_g_a;
    MbtEncoder encoder_msssim_h_a;
    MbtEncoder encoder_msssim_h_s;
    compress::EntropyCoder entropy_msssim;
    ...
    int max_packed_bytes = 560;  // 2 x 280B message payload chunks
...
static std::string _ga_backend(const compressor_config_t* config) {
    if (!config->ga_backend || !config->ga_backend[0]) return "openvino";
    return std::string(config->ga_backend);
}

static std::string _codec_suffix(const compressor_s* ctx) {
...
compressor_t* compressor_create(const compressor_config_t* config) {
    auto* ctx = new compressor_s();
    ctx->config = *config;
    ...
    try {
        ...
        std::cerr << "compress sender contract: codec=" << (codec == 1 ? "msssim_qvrf" : "mbt")
        ...
        std::cerr << "compressor_create failed: " << e.what() << std::endl;
        delete ctx; return nullptr;
    } catch (...) {
        std::cerr << "compressor_create failed: unknown exception" << std::endl;
        delete ctx; return nullptr;
    }
}
...
static float _rc_persist_cap(const compressor_s* ctx) {
...
static float _next_beta_after_frame(const compressor_s* ctx,
                                    float start_beta,
                                    float frame_beta,
                                    int packed_bytes,
                                    bool over_budget) {
...
static void _entropy_encode_with_beta(
    compressor_s* ctx, const float* y_data, float beta,
...
static float _next_gain_after_frame(const compressor_s* ctx,
                                     float start_gain,
                                     float frame_gain,
                                     int packed_bytes,
                                     bool over_budget) {
...
static float _qvrf_retry_gain(const compressor_s* ctx,
                              float gain,
                              int bits,
                              int packed_bytes) {
...
static void _entropy_encode_qvrf(
    compressor_s* ctx, const float* y_data, float gain,
    std::vector<std::string>& y_strings_out,
    std::vector<std::string>& z_strings_out,
    int& z_h, int& z_w)
{
...
int compress_frame(compressor_t* ctx,
    ...
    try {
        ...
        if (use_qvrf) {
            ...
            auto packed = make_msssim_qvrf_message(y_str, z_str, zh, zw, gain);
            ...
            if (bits > ctx->budget_bits || (int)packed.size() > ctx->max_packed_bytes) {
                ...
                packed = make_msssim_qvrf_message(y_str, z_str, zh, zw, gain);
            }
        ...
        ctx->last_stats.rc_passes = 1;
        int bits = _count_bits(y_str) + _count_bits(z_str);
        auto packed = make_strings_message(y_str, z_str, zh, zw, beta, 1);
        ...
        if (bits > ctx->budget_bits || (int)packed.size() > ctx->max_packed_bytes) {
            ...
            ctx->last_stats.rc_passes = 2;
            bits = _count_bits(y_str) + _count_bits(z_str);
            packed = make_strings_message(y_str, z_str, zh, zw, beta, 1);
        }
...
const compress_stats_t* compressor_last_stats(compressor_t* ctx) {
    if (!ctx) return nullptr;
    return &ctx->last_stats;
}

void compressor_destroy(compressor_t* ctx) { delete ctx; }
