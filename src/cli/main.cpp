...
static bool open_shm_with_retry(compress::ShmRing& shm_ring, const char* shm_name) {
    std::string error;
...
static void usage(const char* prog) {
    fprintf(stderr,
        ...
        "  --help                Show this help\n"
        "\n"
        "Sends compressed video frames over serial using 0x0310 message.\n"
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--mqtt-host") == 0) {
            if (++i < argc) mqtt_host = argv[i];
        } else if (strcmp(argv[i], "--mqtt-port") == 0) {
            if (++i < argc) mqtt_port = atoi(argv[i]);
        } else if (strcmp(argv[i], "--mqtt-topic") == 0) {
            if (++i < argc) mqtt_topic = argv[i];
        } else if (strcmp(argv[i], "--mqtt-client-id") == 0) {
            if (++i < argc) mqtt_client_id = argv[i];
        } else if (strcmp(argv[i], "--mqtt-qos") == 0) {
            if (++i < argc) mqtt_qos = atoi(argv[i]);
    ...
    const bool mqtt_output = mqtt_host != nullptr && mqtt_host[0] != '\0';
    if (ipc_output && mqtt_output) {
        fprintf(stderr, "--ipc-host and --mqtt-host are mutually exclusive\n");
        return 1;
    }
    if (mqtt_port <= 0 || mqtt_port > 65535) {
        fprintf(stderr, "--mqtt-port must be in [1,65535]\n");
        return 1;
    }
    if (mqtt_qos < 0 || mqtt_qos > 2) {
        fprintf(stderr, "--mqtt-qos must be 0, 1, or 2\n");
        return 1;
    }

    // --- Open input ---
    FILE* raw_input = nullptr;
    compress::ShmRing shm_ring;
    ...
    if (input_file) {
        ...
        const compress::ShmRingHeader* h = shm_ring.header();
        if (h->pixfmt != compress::SHM_PIXFMT_BGR8 || h->stride < h->width * 3) {
    ...
    compressor_t* compressor = compressor_create(&cfg);
    if (!compressor) {
        if (strcmp(tx_ga_backend, "tensorrt") == 0) {
            fprintf(stderr, "Failed to create compressor. Check:\n"
                            "  - compress was built with TensorRT/CUDA support?\n"
    ...
    rmcc_sniper::mqtt::MqttCustomByteBlockSender mqtt_sender(
        mqtt_host ? mqtt_host : "",
        mqtt_port,
    ...
    if (ipc_output) {
        ...
        if (!mqtt_sender.open()) {
            fprintf(stderr, "Failed to open MQTT %s:%d topic=%s: %s\n",
                    mqtt_host, mqtt_port, mqtt_topic, mqtt_sender.last_error().c_str());
            compressor_destroy(compressor);
            if (raw_input) fclose(raw_input);
            return 1;
        }
    ...
    std::vector<compress_stats_t> profile_stats;
    ...
    for (int frame_id = 0; !g_stop.load() && (num_frames <= 0 || frame_id < num_frames); frame_id++) {
        ...
        if (raw_input) {
            ...
            if (fh.timestamp_ns > 0) {
                const uint64_t now_ns = compress::monotonic_time_ns();
        ...
        if (profile) {
            if (const compress_stats_t* st = compressor_last_stats(compressor)) {
        ...
        if (ret != 0) {
            ...
            over_budget = true;
            over_budget_frames++;
            auto marker = make_over_budget_message(
        ...
        auto chunks = single_packet_mode
            ? make_frame_message(static_cast<uint32_t>(frame_id),
                         bitstream.data(), bitstream_len, stream_id)
            : ((fec_data_chunks > 0)
                ? make_frame_message_fixed_n(static_cast<uint32_t>(frame_id),
...
finish_frames:
    ...
    if (tail_flush_chunks > 0 && !ipc_output && !mqtt_output) {
    ...
    if (ipc_output) {
        ...
        printf("MQTT stats:   %d payload bytes, %d publishes\n",
               total_bytes.load(), total_chunks.load());
    } else if (!dry_run) {
